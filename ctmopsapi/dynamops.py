...
def getinline ():
    ...
    with open('/Users/ncullum/git/ctmopsapi/ctmopsapi/myout.json') as json_file:
        ...
        myajf = mydata["returned"]
        
        
        if myajf == 0:
            print("There are no records found in myout.json. This means there are no jobs available for processing")
            ...
            for uniqid in mydata['statuses']:
                ...
                jobstart = uniqid['startTime']
                if uniqid['name'] == "ncu-apiajf1" and jobstart == "":
                    print("Cannot continue as job status is %s, Exiting pipeline")%(jobstatus)
                    exit()
            ...
            print("Warning : Please run a sanity check for jobnames and details as there may be data to process")
