...
def datarefresh ():
    ...
    #os.system(('ctm run jobs:status::get -s "ctm=ctmaws&folder=%s"')%(jobfolder))
...
def getinline ():
    ...
    with open('/Users/ncullum/eclipse-workspace/ctmopsapi/myout.json') as json_file:
        ...
        #mydata = commands.getoutput('ctm run jobs:status::get -s "ctm=ctmaws&folder=ncu-ajfmanip"')
        mydata = json.loads(json_file.read())
        #mydata = mydata.decode("utf-8")
        #pprint.pprint(mydata)
        #json_file = os.system ('ctm run jobs:status::get -s "ctm=ctmaws&folder=ncu-ajfmanip"')
        jobid = ""
        timecalc = 0
        jobstatus = ""
        for uniqid in mydata['statuses']:
            if uniqid['name'] == "ncu-apiajf1" and timecalc < int(uniqid["startTime"]): #and uniqid['status'] == "Ended OK":
                ...
                if jobstatus == "Ended OK":
                    ...
                    jname = uniqid['name']
                    os.system(('ctm run job:output::get %s > /Users/ncullum/eclipse-workspace/ctmopsapi/oparchive/%s'+'-run-%s.txt' )%(jobid,jname,runcount))
